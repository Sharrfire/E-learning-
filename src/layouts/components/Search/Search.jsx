...
export default function Search(props) {
  ...
  const listCourseFillter = (searchValue,courseList)=>{
    return courseList.filter((values)=>{
      return values.title.toUpperCase().includes(searchValue.toUpperCase());
    })
  }
  useEffect(() => {
    ...
    const fetchApi = async () => {
      ...
      try {
        ...
        setSearchResult(listCourseFillter(searchValue,data));
        setLoading(false);
      } catch (error) {
          ...
          <div className={cx("result-wrapper")}>
            <div className={cx("result-header")}>
              {loading&&<svg
                ...
                data-prefix="fas"
                data-icon="spinner"
                className={cx("svg-inline--fa", " fa-spinner", "search-icon","spin")}
                role="img"
            ...
            {searchResult.map((result)=>
               <NavLink to={`/detail/${result.id}`} key={result.id} className={cx("search-item")} >
